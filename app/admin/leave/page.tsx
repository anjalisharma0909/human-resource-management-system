...
import { Calendar, Check, X, CalendarDays, Clock, FileText, Send, User } from 'lucide-react';
...
export default function LeavePage() {
  ...
  const handleReviewLeave = async (id: string, status: 'Approved' | 'Rejected') => {
    try {
      const response = await authAxios.put(`/api/leave?id=${id}`, { status });
      if (response.data.success) {
    ...
    <DashboardLayout>
      <div className="mb-6">
        ...
        <p className="text-slate-400 text-xs mt-0.5">
          {user?.role === 'Employee' ? 'Submit leave requests and review balances' : 'Inspect and review staff leave submissions'}
        </p>
      </div>
        ...
        <div className="space-y-6">
          ...
          <div className="glass-panel rounded-3xl p-6 border border-white/5">
            ...
            {loading ? (
              ...
              <div className="py-12 text-center text-slate-500 text-xs">
                <Check className="w-8 h-8 text-emerald-400 bg-emerald-500/10 border border-emerald-500/20 rounded-full p-1.5 mx-auto mb-3" />
                <p className="font-bold text-slate-300">All caught up!</p>
                <p className="text-slate-500 text-[10px] mt-0.5">No pending leave applications in the queue.</p>
              </div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {pendingRequests.map((req) => (
                  <div key={req.id} className="bg-white/2 border border-white/5 hover:border-white/10 rounded-2xl p-5 flex flex-col justify-between transition-all">
                    <div>
                      <div className="flex justify-between items-start mb-3">
                        <div className="flex items-center gap-2.5">
                          <div className="w-8 h-8 rounded-full bg-violet-600/10 border border-violet-500/20 text-violet-400 flex items-center justify-center">
                            <User size={14} />
                          </div>
                          <div>
                            <p className="text-xs font-bold text-white">{req.employeeName}</p>
                            <p className="text-[10px] text-slate-500 font-semibold uppercase tracking-wider">{req.leaveType} Leave</p>
                          </div>
                        </div>
                        <span className="text-[10px] text-slate-500 font-mono font-bold">{req.id}</span>
                      </div>

                      <div className="space-y-2 mb-4">
                        <div className="flex gap-1.5 text-xs text-slate-400">
                          <Calendar size={14} className="text-slate-500" />
                          <span className="font-semibold text-slate-300">{req.startDate}</span> to <span className="font-semibold text-slate-300">{req.endDate}</span>
                        </div>
                        <div className="flex gap-1.5 text-xs text-slate-400">
                          <FileText size={14} className="text-slate-500 shrink-0 mt-0.5" />
                          <p className="leading-relaxed text-slate-300 italic">"{req.reason}"</p>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="flex gap-2.5 pt-3 border-t border-white/5 justify-end">
                      <button
                        ...
                        className="px-3 py-1.5 rounded-lg border border-red-500/20 text-red-400 hover:text-white hover:bg-red-500/20 text-xs font-semibold flex items-center gap-1.5 transition-all cursor-pointer"
                      >
                        <X size={12} />
                        Reject
                      </button>
                      <button
                        ...
                        className="px-3 py-1.5 rounded-lg border border-emerald-500/20 text-emerald-400 hover:text-white hover:bg-emerald-500/20 text-xs font-semibold flex items-center gap-1.5 transition-all cursor-pointer"
                      >
                        <Check size={12} />
                        Approve
                      </button>
          ...
          <div className="glass-panel rounded-3xl p-6 border border-white/5">
            <h3 className="text-sm font-bold text-white mb-4">Leave Application Archive</h3>
                ...
                <table className="w-full text-left text-xs border-collapse">
                  ...
                  <tbody className="divide-y divide-white/5">
                    {historyRequests.map((r) => (
                      <tr key={r.id} className="text-slate-300 hover:bg-white/1 transition-colors">
                        <td className="py-3 pr-4 font-mono font-bold text-slate-500">{r.id}</td>
                        <td className="py-3 pr-4 font-bold text-white">{r.employeeName}</td>
                        <td className="py-3 pr-4 font-bold text-slate-400">{r.leaveType}</td>
                        <td className="py-3 pr-4 text-slate-400">
                          {r.startDate} to {r.endDate}
                        </td>
                        <td className="py-3 pr-4 truncate max-w-[150px]" title={r.reason}>{r.reason}</td>
                        <td className="py-3 pr-4 text-slate-400">{r.reviewedBy || 'System'}</td>
                        <td className="py-3">
                          <span
                            className={`px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider ${
                              r.status === 'Approved'
