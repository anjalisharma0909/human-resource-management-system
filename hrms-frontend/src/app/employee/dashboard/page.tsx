...
//         const masterRes = await fetch(`${API_LEAVE}/admin/leaves`); 
//         const backupRes = await fetch(`${API_LEAVE}`); 
...
//             <div>
//               <h1 className="font-bold text-base leading-none text-white tracking-wide">EMPLOYEE CENTER</h1>
//             </div>
//           </div>
...
//               </h3>
//               <p className="text-xs text-gray-500 mb-6">Hello {currentEmpName}.</p>
...
export default function EmployeeDashboard() {
  ...
  const currentEmpName = (resolvedUser as any)?.name || (resolvedUser as any)?.username || (resolvedUser as any)?.employee_name || "Divit";
  const employeeId = (resolvedUser as any)?.id || (resolvedUser as any)?.emp_id || (resolvedUser as any)?.user_id || (resolvedUser as any)?.employee_id || 1; 
  ...
  // --- LIVE CLOCK & INITIAL CORE FETCH DATA ---
useEffect(() => {
    const updateClock = () => {
      const now = new Date();
      const options: Intl.DateTimeFormatOptions = { 
    ...
    async function fetchEmployeeData() {
      ...
      try {
        ...
        if (statsRes.ok) {
          const data = await statsRes.json();
          setEmpStats(data);
          setLeaveBalance(data.remaining_leaves || 24);
          if (data.clocked_in) {
            setClockStatus({ clockedIn: true, checkInTime: data.check_in_time || "Logged" });
          }
        }
        await fetchMyLeavesData(employeeId);
      } catch (err) {
  ...
  const handleClockIn = async () => {
    ...
    const url = "http://127.0.0.1:8000/api/v1/attendance/clock-in";
    let currentTimeString = new Date().toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit', hour12: true });

    try {
      console.log(`Sending Clock-In request to: ${url}`);
      ...
      if (res.ok) {
        setClockStatus({ clockedIn: true, checkInTime: currentTimeString });
        alert("Clocked in successfully!");
      } else {
        alert("Server failed to log Clock-In.");
      }
    } catch (err) {
  ...
  const handleClockOut = async () => {
    ...
    const url = "http://127.0.0.1:8000/api/v1/attendance/clock-out";

    try {
      console.log(`Sending Clock-Out request to: ${url}`);
      const res = await fetch(url, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(payload)
      ...
      if (res.ok) {
        ...
        alert("Server failed to log Clock-Out.");
      }
    } catch (err) {
      console.error("Network error:", err);
      alert("Connection to server failed.");
