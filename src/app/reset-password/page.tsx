  ...
  return (
    <div className="min-h-screen bg-[#0d0e12] flex items-center justify-center p-4">
      <div className="bg-[#14161f] p-8 rounded-xl border border-gray-800 w-full max-w-sm shadow-2xl">
        <h2 className="text-white text-2xl font-bold mb-1 text-center">Reset Password</h2>
        <p className="text-gray-400 text-xs text-center mb-6">Paste your token and set a new password</p>
        ...
        {message && (
          <div className="bg-green-500/10 text-green-400 border border-green-500/20 p-2.5 rounded text-xs mb-4 text-center">
            {message}
          </div>
        )}
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="text-xs text-gray-400 block mb-1">Reset Token</label>
            <input
              type="text"
              required
              value={token}
              onChange={(e) => setToken(e.target.value)}
              className="w-full p-2.5 bg-[#1c1f2b] border border-gray-700 rounded-lg text-white text-xs font-mono focus:outline-none focus:border-purple-500"
