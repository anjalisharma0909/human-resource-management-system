...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // ✅ CORRECT: Send email and password to login endpoint
      const res = await fetch("http://127.0.0.1:8000/api/v1/auth/login", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      // Pass the correct name to AuthContext
      login(
        data.access_token,
        data.refresh_token,
        data.role || "Admin",
      ...
      <div className="w-full max-w-md rounded-2xl bg-[#14161f] p-8 shadow-xl border border-gray-800">
        ...
        <div className="mt-4 text-center">
          <button
            ...
            className="text-xs text-purple-400 hover:text-purple-300 hover:underline transition bg-transparent border-none outline-none cursor-pointer"
          >
